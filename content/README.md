...
/model                         打开菜单
/model list                    列表与密钥状态
/model models [name]           打开 OpenRouter / Ollama 实时模型列表
...
菜单里直接输入可按名称或说明筛选当前列表；Backspace 编辑筛选，Esc 先清空筛选、再返回。Enter 切换/选择，→ 管理，← 返回。

配置 OpenRouter 后，Cawki 会在后台同步官方模型目录。主代理模型选择器只列出支持文本输出和工具调用的模型，并显示上下文长度与视觉能力；Router 分类器不携带工具，因此可选全部文本模型。目录缓存 6 小时；刷新失败时最多使用 7 天内的缓存，再回退到内置精选列表。当前手工配置的模型始终保留。
